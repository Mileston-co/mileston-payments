...
export class RecurringPayment extends BaseAPI {
     ...
     * @private
     */
    private static defaultBaseURL = "https://recurring-service.mileston.co/recurring-payment";
     ...
     */
    async create(businessName: string, payload: CreateRecurringPaymentPayload): Promise<CreateRecurringPaymentResponse> {
        return this.request({
   ...
   */
     ...
     */
    async update(payload: UpdateRecurringPaymentPayload): Promise<UpdateRecurringPaymentResponse> {
        return this.request({
            endpoint: `/update`,
            method: "PATCH",
            data: payload,
